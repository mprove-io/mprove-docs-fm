...
Store defines fields and logic for retrieving data via an HTTP request to a third-party API provider.

For each store, Mprove creates a model with a name starting with the prefix "**store_model\_**".

Provided example has been tested for Google Analytics Reporting API, but the approach itself is generic and should work for most other API providers.
