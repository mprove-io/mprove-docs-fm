...
User can see combined schemas in UI and explore Relationships Graph.
...
Raw schemas are cached. User must click "Refresh" button in UI to refresh a raw schema cache. Raw schema cache is not refreshed on page load.
...
This way Malloy does not need to fetch data from db. It improves Malloy compilation time significantly, because Malloy fetches data from db on a per table basis. The only exception is when Malloy needs to sample data (some SnowFlake datatypes).
