...
| ------------ | -------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| model        | tag      | -       | If specified, malloy Source will be available for exploration as mprove Model.                                                                           |
| access_roles | string[] | -       | Only users with the specified roles will be able to explore the model. If `access_roles` tag is not specified, all project users will be able to explore the model. |

### Dimension (mprove) Tags

| Name          | Type     | Default | Description                                                                                                      |
| ------------- | -------- | ------- | ---------------------------------------------------------------------------------------------------------------- |
| build_metrics | tag      | -       | Metrics will be created for each time based dimension with `build_metrics` tag and all measures of mprove Model. |
| field_group   | string[] | -       | Fields will be added to a group in the web UI (model fields tree).                                               |
