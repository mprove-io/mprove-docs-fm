...
It is important to store malloy files containing `#(mprove) model` separately from other
malloy elements, such as `run: ...` or `query: ...`, in order to minimize malloy compilation time.
This file structure also improves readability and simplifies maintenance.
